...
vector<worder> mergeNegNumber(vector<worder> worders)  //合并负号和数字
{
    vector<worder> result;
    for(int i = 0; i < worders.size(); i++) {
        worder word = worders[i];
        if((i == 0 || worders[i-1].type == OPERATER && worders[i-1].name != ")" && worders[i-1].name != "(" ) && (worders[i].name == "+" || worders[i].name == "-") && worders[i+1].type == NUMBER) {
            word.name += worders[++i].name;
            word.type = NUMBER;
        }
        result.push_back(word);
...
{
    vector<worder> result = mergeNegNumber(mergeFunction(judgeWord(separateSentence(sentence))));
    return result;
}
vector<vector<worder> > processArgumentsOfFunction(worder word) //将函数的参数合并为表达式
{
    ...
    if(wordersofargument.empty()) return vector<vector<worder> >(1);
