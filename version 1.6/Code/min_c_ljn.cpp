...
int JudgeLine (string tobeJudged) {
	...
	if (jgSize > 2 && tobeJudged[0] == 'd' && tobeJudged[jgSize-1] == '{') {
		if (tobeJudged[1] == 'o') {
			nowLine.push_back('o');
			if (tobeJudged[2] == '{')
				return 0;
			else if (tobeJudged[2] == ' ') {
				if (tobeJudged[3] == '{')
					return 0;
				else {
					errLocation[0] = ansLocation.size() - 1, errLocation[1] = 3;
					errInformation = "Do syntax error.";
					return 1;
				}
			}
			else {
	...
	if (jgSize > 6 && tobeJudged[0] == 'u' && tobeJudged[jgSize-1] == ';') {
		if (tobeJudged[1] == 'n' && tobeJudged[2] == 't' && tobeJudged[3] == 'i' && tobeJudged[4] == 'l' && tobeJudged[5] == ' ') {
			if (tobeJudged[6] == ';') {
				errLocation[0] = ansLocation.size() - 1, errInformation[1] = 6;
				errInformation = "Until syntax error.";
				return 1;
			}
			nowLine.push_back('p');
			return 0;
...
vector <string> ReceiveFile (string pendingFile) {
	...
	if (ans.size() == 0)
		...
		throw Compile_E(ansLocation[errLocation[0]][0], errInformation);
	return ans;
}
