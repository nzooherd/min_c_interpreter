...
using namespace std;
CPU::CPU() : Functions(),Scopes(),Stacks(),layerNum(0),ret('i'),MainOps(),returnFlag(false){
}
...
Var& CPU::getVar_(const string&name) {
	...
	map<string, Var >::iterator it = findVar(name);
	if (it == Scopes[layerNum - 1].vars.end())
...
map<string, Var>::iterator CPU::findVar(const string& name) {
	return findVar(name, layerNum - 1);
}
map<string,Var>::iterator CPU::findVar(const string& name,int layer){
	int ori_layer = layer;
	...
	do{
		...
		if (it != Scopes[layer].vars.end())
			...
			return Scopes[ori_layer].vars.end();
	} while (true);
}
