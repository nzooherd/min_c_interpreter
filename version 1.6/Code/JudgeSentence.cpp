...
int judgeFunction(const vector<worder>& worders)    //判断是否属于函数语句 返回1表示声明函数，返回2表示调用函数,返回0表示非函数语句
{
    string sentence;
    int result;
    worder funt;
	...
	vector<worder> wordersOfArgument = complateSepSentence(sentence);
    if(result == 1){
        for(int i = 0; i < wordersOfArgument.size(); i++) {
			if ((i % 3 == 0 && wordersOfArgument[i].type != DECLARTION) ||
				(i % 3 == 1 && wordersOfArgument[i].type != IDENTIFIER) ||
				(i % 3 == 2 && wordersOfArgument[i].name != ","))
				break;
            if(i == wordersOfArgument.size() - 1) return 1;
        }
        return 0;
    }
	for(auto arg: processArgumentsOfFunction(worders[0]))
        if(!judgeExpression(arg)) return 0;
    return 2;
}
bool judgeUntil(vector<worder> worders)
{
	if (worders[0].name != "until") return false;
	vector<worder>::iterator k = worders.begin();
	worders.erase(k);
	worders.pop_back();
	if (!judgeExpression(worders)) return false;
	return true;
}
