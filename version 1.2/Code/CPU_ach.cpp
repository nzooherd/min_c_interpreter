...
using namespace std;
CPU::CPU() : Functions(), Scopes(), Stacks(), layerNum(0), ret('i'), op_line(0),
MainOps(), returnFlag(false), wait(), tempWait(-2), nowLine(-1), runnable(true), origin_ops(),
line_map_fm(), line_map_fo(), line_map_of(), maxOriginLine(0),lastOriginLine(-1)
{
}
...
void CPU::WHILE(const vector<pair<int,string>>&ops, int &start) {
	...
	while (start < ops.size() && !(ops[start].second[0] == '0' && ops[start].second[1] == 'W'))
		part1.push_back(pair<int,string>(ops[start].first,ops[start++].second));
	string* name = nullptr;
